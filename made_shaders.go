...
func loadGaussian127Shader() (*gaussian127Shader, error) {
	var vs, fs, program uint32

	{
		vs = gli.CreateShader(gl_VERTEX_SHADER)
		gli.ShaderSource(vs, gaussian127VS)
		gli.CompileShader(vs)

		shLog := gli.GetShaderInfoLog(vs)
		if len(shLog) > 0 {
			fmt.Printf("VERTEX_SHADER compilation log for gaussian127VS:\n\n%s\n", shLog)
		}
		...
		if status != gl_TRUE {
			gli.DeleteShader(vs)
			return nil, errors.New("Error compiling GL_VERTEX_SHADER shader part for gaussian127VS")
		}
		if glErr := gli.GetError(); glErr != gl_NO_ERROR {
			return nil, errors.New("error compiling shader part for gaussian127VS, glError: " + fmt.Sprint(glErr))
		}
	}

	{
		fs = gli.CreateShader(gl_FRAGMENT_SHADER)
		gli.ShaderSource(fs, gaussian127FS)
		gli.CompileShader(fs)

		shLog := gli.GetShaderInfoLog(fs)
		if len(shLog) > 0 {
			fmt.Printf("FRAGMENT_SHADER compilation log for gaussian127FS:\n\n%s\n", shLog)
		}
		...
		if status != gl_TRUE {
			gli.DeleteShader(fs)
			return nil, errors.New("Error compiling GL_FRAGMENT_SHADER shader part for gaussian127FS")
		}
		if glErr := gli.GetError(); glErr != gl_NO_ERROR {
			return nil, errors.New("error compiling shader part for gaussian127FS, glError: " + fmt.Sprint(glErr))
		}
	}

	{
		...
		shLog := gli.GetProgramInfoLog(program)
		if len(shLog) > 0 {
			fmt.Printf("Shader link log for gaussian127FS:\n\n%s\n", shLog)
		}
		...
		if status != gl_TRUE {
			gli.DeleteShader(vs)
			gli.DeleteShader(fs)
			return nil, errors.New("error linking shader for gaussian127FS")
		}
		if glErr := gli.GetError(); glErr != gl_NO_ERROR {
			return nil, errors.New("error linking shader for gaussian127FS, glError: " + fmt.Sprint(glErr))
		}
	}

	result := &gaussian127Shader{}
	result.id = program
	result.vertex = uint32(gli.GetAttribLocation(program, "vertex"))
